...
public class DefaultMovieService : IMovieService
{
	...
	public async Task<Result<Movie>> UpdateMovie(int id, Movie newData)
	{
		...
		var movie = await movieRepository.UpdateMovie(id, newData); var result = movie == null ? new Result<Movie>(new Exception($"Could not update movie {newData} with id {id}."), (int)HttpStatusCode.NotFound) : new Result<Movie>(movie, (int)HttpStatusCode.OK);
	...
	{
		var movie = await movieRepository.DeleteMovie(id); var result = movie == null ? new Result<Movie>(new Exception($"Could not delete movie with id {id}."), (int)HttpStatusCode.NotFound) : new Result<Movie>(movie, (int)HttpStatusCode.OK);
